...
public class IdlingResourceMenuActivityTest {
    ...
    // (6) Registering any resource that needs to be synchronized with Espresso before
    // the test is run.
    @Before
    ...
    //(7) Testing that the gridView with Tea objects appears and we can click a gridView item
    @Test
    public void idlingResourceTest() {
        onData(anything()).inAdapterView(withId(R.id.tea_grid_view)).atPosition(0).perform(click());
    }

    // (8) Unregistering resources when not needed to avoid malfunction
    @After
